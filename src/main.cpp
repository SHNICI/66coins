...
// Copyright (c) 2014 66 Maintainer
...
uint256 hashGenesisBlock("0xf2ab5b325627cd0df6a17f4e6ad87abf2c61d033c9bfd2f09144f43db5ff4dee");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // 66: starting difficulty is 1 / 2^12
...
const int SECOND_HARDFORK_BLOCK_HEIGHT = 580364;
const double BONUS_REWARD_FACTOR = 5.6;
const int64 MAX_COINS = 100 * COIN;
...
static int64 GetBlockValue(int nHeight, int64 nFees, CBlockIndex *prevBlock)
{
	...
	if (nHeight > 250 && nHeight < 33250)
		nSubsidy = 0.000066 * COIN;
	
	if (nHeight > SECOND_HARDFORK_BLOCK_HEIGHT) // 580,364
		nSubsidy = 0.000066 * COIN;
		
	// Reward reduces 5% yearly	
	if (nHeight < 580364) 
	{
		int yearsElapsed = nHeight / 1000000;
...
bool DeservesBonusReward(CBlockIndex* pindex)
{
	...
	if(pindex->nMoneySupply > 100.999 * COIN)
		return false;
...
static const int64 nTargetSpacing = 66;
static const int64 nInterval = nTargetTimespan / nTargetSpacing;
static const int64 nTargetTimespanRe = 660;
static const int64 nTargetSpacingRe = 66;
...
static unsigned int GetNextWorkRequiredBeforeKimoto(const CBlockIndex *pindexLast, const CBlock *pblock)
{
	...
	// 66: This fixes an issue where a 51% attack can change difficulty at will.
	...
	if (fNewDifficultyProtocol) {
		if (nActualTimespan < (retargetTimespan - (retargetTimespan / 10)))
			nActualTimespan = (retargetTimespan - (retargetTimespan / 10));
		if (nActualTimespan > (retargetTimespan + (retargetTimespan / 10)))
			nActualTimespan = (retargetTimespan + (retargetTimespan / 10));
	} else {
...
static unsigned int GetNextWorkRequiredKimoto(const CBlockIndex *pindexLast, const CBlock *pblock)
{
	...
	int64 PastSecondsMin = TimeDaySeconds * 0.55;
	int64 PastSecondsMax = TimeDaySeconds * 1.89;
...
static unsigned int GetNextWorkRequired(const CBlockIndex *pindexLast, const CBlock *pblock)
{
	if ((pindexLast->nHeight > FIRST_HARDFORK_BLOCK_HEIGHT) || fTestNet)
		...
		return GetNextWorkRequiredBeforeKimoto(pindexLast, pblock);
}
                                 ...
                                 const CBlockIndex *pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
	...
	// fMiner is true when called from the internal 66 miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
	...
	if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes) {
		...
		printf("*** %s\n", strMessage.c_str());
		uiInterface.ThreadSafeMessageBox(strMessage, "66", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
		StartShutdown();
		return false;
	}
	return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
	if (fTestNet) {
		...
		hashGenesisBlock = uint256("0x505c40ef0f4efad59c050d4f0e13cede8b454ab18ac98f2414b92f088d52705f");
	}
	...
	if (mapBlockIndex.empty()) {
		...
		// block.hashMerkleRoot = 4fe8c1ba0a102fea0643287bb22ce7469ecb9b690362013f269a663fefa77b6e
		...
		//     CTxIn(COutPoint(0000000000, -1), coinbase 04ffff001d01044c534d61792031332c20323031332031313a3334706d204544543a20552e532e2063727564652066757475726573207765726520757020302e332070657263656e76606176602439352e343120612062617272656c)
		//     CTxOut(nValue=50.00000000, scriptPubKey=040184710fa689ad5023690c80f3a4)
		//   vMerkleTree: 4fe8c1ba0a

		// Genesis block
		const char *pszTimestamp = " Sixty Six Coin ";
		CTransaction txNew;
		txNew.vin.resize(1);
		txNew.vout.resize(1);
		txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char *)pszTimestamp, (const unsigned char *)pszTimestamp + strlen(pszTimestamp));
		txNew.vout[0].nValue = 1 * COIN;
		txNew.vout[0].scriptPubKey = CScript() << ParseHex("04C18FCBE8A9D2D2922D455E7E82F7733EC9DABA8E15C7EDB2A3164F6C9BF954ED4390DF811901DA6D4B7C984ED083FBDF360C47704DCB607CE96460E58850310C") << OP_CHECKSIG;
		CBlock block;
		block.vtx.push_back(txNew);
		block.hashPrevBlock = 0;
		block.hashMerkleRoot = block.BuildMerkleTree();
		block.nVersion = 1;
		block.nTime = 1390340839;
		...
		assert(block.hashMerkleRoot == uint256("0x470da0811b0d4ac267fb05789065eb634f424002a1e308b04cd8ef4d9dceaff7"));
...
{
	return false;
	//commenting out this code until we properly implement 66 alerts.
	/*
    if (!CheckSignature())
        return false;
    if (!IsInEffect())
...
CBlock *CreateNewBlock(CReserveKey &reservekey)
{
	...
	int64 nFees = 0;
	{
		...
		while (!mapPriority.empty()) {
			...
			// 66: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
...
static unsigned int KimotoGravityWell(const CBlockIndex *pindexLast, const CBlock *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax)
{
	...
	for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
		...
		EventHorizonDeviation = 1 + (0.7084 * pow((double(PastBlocksMass) / double(215)), -1.228));
