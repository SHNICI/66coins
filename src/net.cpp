...
bool CNode::Misbehaving(int howmuch)
{
	...
	if (nMisbehavior >= GetArg("-banscore", 100)) {
		int64 banTime = GetTime() + GetArg("-bantime", 60 * 1); // Default 1 minutes ban
		{
			LOCK(cs_setBanned);
...
void ThreadMapPort2(void *parg)
{
	...
	if (r == 1) {
		...
		string strDesc = "66 " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
		/* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    {"ns1.afraid.org", "seed.flowpool.cf"},
	{"flowpool.ddns.net:36691", "flowpool.no-ip.org"},
    {NULL, NULL}
};
...
// Create socket for listening for incoming connections
	...
	if (::bind(hListenSocket, (struct sockaddr *)&sockaddr, len) == SOCKET_ERROR) {
		...
		if (nErr == WSAEADDRINUSE)
			strError = strprintf(_("Unable to bind to %s on this computer. 66 is probably already running."), addrBind.ToString().c_str());
		else
